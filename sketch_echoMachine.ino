/*
  echo machine source code~~Harry Morley Dec 2018

  echo_check() modified from "NewPingEventTimer" example from the NewPing library in order to not use delays in the sketch.
  ...
  if (sonar.check_timer()) { //Calculate distance and start selecting pattern, if ping received.
    ...
    if (dist < 40) {
      if (listening) {
        ...
        selecting = true;
        listening = false; // ignore further input for 5sec
      }
    }
  }
